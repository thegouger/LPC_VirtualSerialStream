 ...
 * All rights reserved.
 *
 * Modified to include custom stream setup
 *
 * @par
...
/** Non-blocking read function for custom stream */
ssize_t cdc_read(void* cookie, char* buff, size_t n)
{
    for(uint16_t i = 0; i < n; i++)
    	buff[i] = CDC_Device_getchar((USB_ClassInfo_CDC_Device_t*)cookie);

    return n;
}

/** Blocking read function for custom stream */
ssize_t cdc_read_blocking(void* cookie, char* buff, size_t n)
{
    for(uint16_t i = 0; i < n; i++)
    	buff[i] = CDC_Device_getchar_Blocking((USB_ClassInfo_CDC_Device_t*)cookie);

    return n;
}

/** Write function for custom stream */
ssize_t cdc_write(void* cookie, const char* buf, size_t n)
{
	CDC_Device_SendData((USB_ClassInfo_CDC_Device_t*)cookie, buf, n);

	return n;
}

FILE* CDC_Device_CreateStream(USB_ClassInfo_CDC_Device_t* const CDCInterfaceInfo)
{
	cookie_io_functions_t cookie;
	cookie.read = cdc_read;
	cookie.write = cdc_write;
	cookie.seek = NULL;
	cookie.close = NULL;

	return fopencookie(CDCInterfaceInfo, "w+", cookie);
}

FILE* CDC_Device_CreateBlockingStream(USB_ClassInfo_CDC_Device_t* const CDCInterfaceInfo)
{
	cookie_io_functions_t cookie;
	cookie.read = cdc_read_blocking;
	cookie.write = cdc_write;
	cookie.seek = NULL;
	cookie.close = NULL;
...
static int CDC_Device_putchar(USB_ClassInfo_CDC_Device_t* const CDCInterfaceInfo, char c)
{
	return CDC_Device_SendByte(CDCInterfaceInfo, c) ? -1 : 0;
}

static int CDC_Device_getchar(USB_ClassInfo_CDC_Device_t* const CDCInterfaceInfo)
{
	int16_t ReceivedByte = CDC_Device_ReceiveByte(CDCInterfaceInfo);
...
static int CDC_Device_getchar_Blocking(USB_ClassInfo_CDC_Device_t* const CDCInterfaceInfo)
{
	int16_t ReceivedByte;

	while ((ReceivedByte = CDC_Device_ReceiveByte(CDCInterfaceInfo)) < 0)
	{
		if (USB_DeviceState[0] == DEVICE_STATE_Unattached)
		  return -1;
